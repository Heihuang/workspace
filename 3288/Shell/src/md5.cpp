...
{
	HANDLE hFile = ::CreateFile(file, GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING, FILE_FLAG_SEQUENTIAL_SCAN, NULL);
	if (hFile == INVALID_HANDLE_VALUE)
		return "";

	HANDLE hMapping = ::CreateFileMapping(hFile, NULL, PAGE_READONLY | SEC_COMMIT, 0, 0, NULL);
	if (!hMapping)
	{
		::CloseHandle(hFile);
		return "";
	...
	LPVOID pBuf = ::MapViewOfFile(hMapping, FILE_MAP_READ, 0, 0, 0);
	if (!pBuf)
	{
